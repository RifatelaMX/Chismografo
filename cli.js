...
// 8. Command: test-domain
  ...
  try {
    ...
    if (attr) {
      if (result[attr] !== undefined) {
        if (Array.isArray(result[attr])) {
          if (result[attr].length > 0 && typeof result[attr][0] === 'object') {
            console.log(result[attr].map(item => item.name).join(', '));
          } else {
            console.log(result[attr].join(', '));
          }
        } else if (typeof result[attr] === 'object') {
          console.log(JSON.stringify(result[attr]));
        } else {
    ...
    if (result.cms) {
      ...
      console.log(`   - ${result.cms} (Confianza: ${(result.confidence * 100).toFixed(2)}%)${themeText}\n`);
    } else {
      console.log('\x1b[36m\x1b[1m%s\x1b[0m', '📦 Plataforma E-Commerce (CMS):');
    ...
    if (result.apps && result.apps.length > 0) {
      result.apps.forEach(app => {
        console.log(`   - [${app.category}] ${app.name} (Desarrollador: ${app.developer || 'N/A'})`);
      });
      console.log('');
    } else {
    ...
    if (result.paymentProcessors && result.paymentProcessors.length > 0) {
      console.log(`   - ${result.paymentProcessors.join(', ')}\n`);
    } else {
    ...
    if (result.paymentMethods && result.paymentMethods.length > 0) {
      console.log(`   - ${result.paymentMethods.join(', ')}\n`);
    } else {
