document.addEventListener('DOMContentLoaded', () => {
  ...
  function loadSettings() {
    if (bwApiKeyBannerInput) bwApiKeyBannerInput.value = localStorage.getItem('rapid_api_key') || '';
  }
  ...
  async function performDetection(targetUrl) {
    ...
    try {
      ...
      scanningState.classList.add('hidden');
      submitBtn.disabled = false;
      showError('Ocurrió un error en la comunicación con la API. Asegúrate de que el servidor está corriendo.');
    }
  }
  ...
  async function fetchPageSpeed(targetUrl) {
    ...
    try {
      ...
      if (data.success) {
        ...
        updateGauge('pagespeed-score-val', 'pagespeed-score-circle', 'pagespeed-level-badge', scores.performance);
        updateGauge('pagespeed-acc-val', 'pagespeed-acc-circle', 'pagespeed-acc-badge', scores.accessibility);
        updateGauge('pagespeed-seo-val', 'pagespeed-seo-circle', 'pagespeed-seo-badge', scores.seo);

        // Metrics values
        if (document.getElementById('ps-metric-fcp')) document.getElementById('ps-metric-fcp').textContent = data.metrics.fcp;
        if (document.getElementById('ps-metric-lcp')) document.getElementById('ps-metric-lcp').textContent = data.metrics.lcp;
        if (document.getElementById('ps-metric-tbt')) document.getElementById('ps-metric-tbt').textContent = data.metrics.tbt;
        if (document.getElementById('ps-metric-cls')) document.getElementById('ps-metric-cls').textContent = data.metrics.cls;
        if (document.getElementById('ps-metric-speedindex')) document.getElementById('ps-metric-speedindex').textContent = data.metrics.speedIndex;
        if (document.getElementById('ps-metric-interactive')) document.getElementById('ps-metric-interactive').textContent = data.metrics.interactive;
  ...
  function renderResults(data) {
    ...
    if (mapCard) {
      if (data.location && data.location.success && data.location.ll) {
        ...
        document.getElementById('server-location-val').textContent = `${city ? city + ', ' : ''}${region ? region + ', ' : ''}${country}`;
    ...
    if (data.detected) {
      ...
      if (techIconContainer) {
        if (cmsDom) {
          ...
          techIconContainer.style.background = 'linear-gradient(135deg, var(--accent-primary) 0%, var(--accent-secondary) 100%)';
  ...
  function renderUnifiedDashboard(data) {
    ...
    const mergeItem = (name, category, link, firstSeen, lastSeen, isPremium, shopifyAppIcon, source) => {
    ...
    if (data.plugins) {
      data.plugins.forEach(p => {
        mergeItem(p.name, p.category, p.evidence.startsWith('http') ? p.evidence : '', '', '', '', p.shopifyAppIcon, 'Motor Local');
      });
    }
    ...
    const shopifyAppsLabel = shopifyAppsMetricCard ? shopifyAppsMetricCard.querySelector('.metric-label') : null;
  ...
  function translateCategory(name) {
    ...
    if (lower === 'payment' || lower === 'payments' || lower === 'consumer finance' || lower === 'billing' || lower === 'pay later' || lower === 'paylater') return 'Métodos de Pago';
    if (lower === 'payment gateways' || lower === 'payment gateway' || lower === 'payment processor' || lower === 'payment processors') return 'Procesador de Pago';
    if (lower === 'hosting' || lower === 'hosting provider' || lower === 'cloud hosting' || lower === 'server location' || lower === 'almacenamiento') return 'Ubicación de Almacenamiento';
    if (lower === 'cms' || lower === 'shop' || lower === 'ecommerce' || lower === 'e-commerce' || lower === 'platform') return 'Plataforma de E-Commerce (CMS)';
    if (lower === 'widgets' || lower === 'widget') return 'Componentes y Widgets';
    if (lower === 'marketing automation' || lower === 'marketing') return 'Automatización de Marketing';
    ...
    if (lower === 'ssl' || lower === 'root authority' || lower === 'certificate') return 'Seguridad y SSL';
