    ...
    files.forEach(file => {
      if (file.endsWith('.json')) {
        try {
          ...
          console.error(`[TechRulesLoader] Failed to parse JSON file ${file} in techs/${folderName}:`, err.message);
        }
      }
    });
  }
...
export function loadAllTechRules() {
  const isDev = process.env.DEV === 'true' || process.env.NODE_ENV === 'development' || process.env.NODE_ENV === 'dev';
  let indexData;
  ...
  console.log(`[TechRulesLoader] Rules loaded and compiled: ${cmsList.length} CMS, ${appsList.length} Apps, ${infraList.length} Infra.`);
}
