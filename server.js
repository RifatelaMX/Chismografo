...
function validateUrlParam(req, res, next) {
  ...
  if (!url) {
    return res.status(400).json({
      success: false,
      error: 'Parameter "url" is required. Provide it in JSON body (POST) or query parameter (GET).'
    });
  }
...
const appLogoCache = {
  'klaviyo-email-marketing': 'https://cdn.shopify.com/app-store/listing_images/5edd9000b933a8fa88c152d1e498531f/icon/CP6B2OOv3PYCEAE=.png',
  'loox-fashion-reviews': 'https://cdn.shopify.com/app-store/listing_images/7bcde5f377484dfde0d59218653c023d/icon/CMLk56rv3PYCEAE=.png',
  'pagefly': 'https://cdn.shopify.com/app-store/listing_images/f77b919d3ee9a3d46cc8397a61d157a9/icon/CNmwtPfv3PYCEAE=.png',
  'klarna-payments': 'https://cdn.shopify.com/app-store/listing_images/2e071c3c9fb605cf52538166c4efbb16/icon/CP3Z_tXv3PYCEAE=.png',
  'conekta': 'https://cdn.shopify.com/app-store/listing_images/1628d085994cd21cda28045d62ee60d8/icon/CKHj2b7v3PYCEAE=.png',
  'mercado-pago': 'https://cdn.shopify.com/app-store/listing_images/b2d416b71ea6ab262024db5e263e8a27/icon/CODrzrvv3PYCEAE=.png',
  'openpay': 'https://cdn.shopify.com/app-store/listing_images/3ce84d28c31e9c20a4843ed2d7b6f6aa/icon/CKXj_7_v3PYCEAE=.png',
  'mailchimp': 'https://cdn.shopify.com/app-store/listing_images/362a28b03e06a3809fb0fa4b1b3b145d/icon/CP6s_rXv3PYCEAE=.png'
};
...
function getShopifyAppSlug(name) {
  ...
  if (nameLower.includes('mercado pago') || nameLower.includes('mercadopago')) return 'mercado-pago';
...
async function getScreenshot(domain, device = 'desktop', extraParams = {}) {
  const cleanDomain = domain.replace(/^https?:\/\//i, '').replace(/^www\./i, '').split('/')[0].trim();
  ...
  try {
    ...
    console.error(`[Screenshot Machine] Failed to fetch screenshot for ${cleanDomain} (${device}):`, err.message);
...
app.post('/api/detect', validateUrlParam, async (req, res) => {
  ...
  if (result.success) {
    ...
    // Add screenshots
    const domain = req.normalizedUrl.replace(/^https?:\/\//i, '').replace(/^www\./i, '').split('/')[0].trim();
    try {
...
function inferPaymentMethods(gateways) {
  ...
  gateways.forEach(gw => {
    ...
    if (gwLower.includes('stripe') || gwLower.includes('adyen') || gwLower.includes('braintree') || gwLower.includes('dlocal')) {
      methods.add('Tarjeta de Crédito / Débito');
    }
    ...
    else if (gwLower.includes('klarna') || gwLower.includes('aplazo') || gwLower.includes('kueski')) {
...
async function getPageSpeedMetrics(url) {
  try {
    ...
    if (!lr) return { success: false, error: 'Respuesta inválida de Google PageSpeed' };
...
app.get('/widget', validateUrlParam, async (req, res) => {
  ...
  const domain = req.normalizedUrl.replace(/^https?:\/\//i, '').replace(/^www\./i, '').split('/')[0];
